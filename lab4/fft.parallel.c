...
int main ( )
{
  ...
  double z1;
  void cfft2 ( int n, double x[], double y[], double w[], double sgn );
  int num_threads = 4; // You can adjust this based on your system
...
*/
  ...
  for ( ln2 = 1; ln2 <= 20; ln2++ )
  {
   ...
   pthread_t* threads = (pthread_t*)malloc(num_threads * sizeof(pthread_t));
    ...
    // Array to store thread-specific data
    ThreadData* thread_data = (ThreadData*)malloc(num_threads * sizeof(ThreadData));

    // Divide the iterations among threads
    int iterations_per_thread = nits / num_threads;
    int remaining_iterations = nits % num_threads;
    n = 2 * n;
